...
class AddPostModal extends Component {
    ...
    test(pic) {
        ...
        fetch(BASE_URL_AGENT + "/api/product/upload", options);
    }
	sendRequestForFeedAlbum(feedPostDTO) {
		

		Axios.post(BASE_URL_AGENT + "/api/product/add" , feedPostDTO, {  headers: { Authorization: getAuthHeader() } })
			.then((res) => {
			
				
				let feedId = res.data;
				this.state.pictures.forEach((pic) => {
					this.test(pic, feedId);
				});
